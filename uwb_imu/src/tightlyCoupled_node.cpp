...
float dt;
Tfusion::TightlyCoupledUwbImu imu_uwb_fuser; // fuser object
ros::Publisher traj_puber;
ros::Publisher result_puber;
bool initialized = false;
Tfusion::ImuData<double> last_uwb_imu;     //last interpolated imu data at uwb time
Tfusion::ImuData<double> last_imu;         //last imu data for predict
Tfusion::STATE last_updated_state; //last updated state by uwb 
Tfusion::ImuData<double> cur_imu;
...
Tfusion::ImuData<double> fromImuMsg(const sensor_msgs::Imu &msg)
{
    Tfusion::ImuData<double> imu_data;
...
Tfusion::UwbData<double> fromUwbMsg(const uwb_imu::UwbMsg &msg)
{
    Tfusion::UwbData<double> uwb_data;
...
void pubResult()
{   
    Tfusion::STATE result = imu_uwb_fuser.getState();
...
void processThread()
{
    ...
    while (ros::ok())
    {
        ...
        if (!initialized)
        {
            ...
            vector<Tfusion::ImuData<double>> imu_datas;
        ...
        for (auto &imu_msg : imu_buffer)
        {
            ...
            {   
                ...
                dt = imu_uwb_fuser.predict(last_imu,cur_imu); 
        ...
        {    
            ...
            vector<Tfusion::ImuData<double>> imu_datas(0);
            ...
            Tfusion::ImuData<double> cur_uwb_imu = fromImuMsg(inter_imu);

            cur_uwb_imu.stamp = cur_stamp;
            imu_datas.push_back(cur_uwb_imu);
            // generate uwb data
            Tfusion::UwbData<double> uwb_data = fromUwbMsg(*uwb_buffer[0]);

            imu_uwb_fuser.update(uwb_data); 
