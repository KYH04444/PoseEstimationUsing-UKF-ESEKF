...
class ESEKF:
    def __init__(self):
        ...
        self.m_matR = 0.08 * np.eye(3, dtype=np.float32)
        self.m_vecZ = np.zeros(3, dtype=np.float32)
    ...
    def setQ(self, dt):
        ...
        self.m_matQ[9:12,9:12] *= 0.0000015387262937311438 * np.eye(3)*dt # 모름
    ...
    def imu_callback(self, msg):
        ...
        imu_data = {
            ...
            "linear_acc": [msg.linear_acceleration.y, -msg.linear_acceleration.x, msg.linear_acceleration.z],
            "angular_vel": [msg.angular_velocity.y, -msg.angular_velocity.x, msg.angular_velocity.z]
        }
    ...
    def process_data(self):
        if len(self.imu_data_queue) > 1 and len(self.uwb_data_queue) > 0:
            ...
            if imu_data_1["timestamp"] <= uwb_data["timestamp"] <= imu_data_2["timestamp"]:
                ...
                # print("uwb time = ",uwb_data["timestamp"])
                # print("imu time = ",imu_data_2["timestamp"])
                alpha = (t - t1) / (t2 - t1)
                ...
                self.m_vecZ = np.array(uwb_data["pos_3d"])
                self.m_vecZ[2] *=-1
                self.prediction()
                self.correction()
    ...
    def correction(self):
        ...
        H[2,2] = 1 
        self.m_jacobian_matH = H @ self.updateH()        
        residual = self.m_vecZ - state_vec
        ...
        self.m_matP = np.dot(np.dot(I_KH, self.m_matP),I_KH.transpose()) + np.dot(np.dot(Kk,self.m_matR),Kk.transpose())
        pose = PoseStamped()
