...
class ReadyToLocalize(object):
    ...
    def printPublishPosition(self, position):
        ...
        if network_id is None:
            network_id = 0
        print("POS ID {}, x(m): {pos.x} y(m): {pos.y} z(m): {pos.z}".format(
        ...
        if self.osc_udp_client is not None:
            ...
            # if position.x is not None:
                # uwb_msg = UwbMsg()
            self.uwb_msg.header.stamp = rospy.Time.now()
            self.uwb_msg.pos_x = int(position.x)
            self.uwb_msg.pos_y = int(position.y)
            self.uwb_msg.pos_z = int(position.z)
            # return(uwb_msg)
...
if __name__ == "__main__":
    ...
    pub = rospy.Publisher("uwb", UwbMsg)
    ...
    anchors = [DeviceCoordinates(0x116F, 1, Coordinates(-1500, 600, 1250)),
               DeviceCoordinates(0x1166, 1, Coordinates(1500, 0, 1250)),
               DeviceCoordinates(0x113B, 1, Coordinates(900, 1200, 1250)),
               DeviceCoordinates(0x1179, 1, Coordinates(300, -900, 1250)),
               DeviceCoordinates(0x1102, 1, Coordinates(-1500, 600, 1250))]
